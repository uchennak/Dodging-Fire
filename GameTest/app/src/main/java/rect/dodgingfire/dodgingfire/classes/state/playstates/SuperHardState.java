 ...
 */
public class SuperHardState extends State {
...
//Checking for collisions with the fire on the ground
        for (Fire fire : lowFires) {
            ...
            if (iceCream3) {
                ...
                if (fire.isVisible() && Rect.intersects(iceCream.getRect(), fire.getRect())) {
                    setCurrentState(new SuperHardGameOverState(playerScore / 100));

                }
            }
        }

//Checking for collisions with the fire on the middle
        for (Fire fire : rAboveLowFires) {
